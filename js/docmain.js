...
var contractAddress_rinkeby = "0xc8d224404d82eedfcb51f108007aa74f201a0aa4";         // Rinkeby
var contractAddress_mainnet = "0xc8d224404d82eedfcb51f108007aa74f201a0aa4";         // Main net
...
function connectToBlockchain() {
    ...
    if (contractAddress !== undefined) {
        ...
        ethblogContract = web3.eth.contract(contractAbi);

        try {
            ethblogInstance = ethblogContract.at(contractAddress);
        }
        catch (err) {
...
function showNetworkNotSupportedError() {
    showModal("Network not supported", "ethblog is available on Mainnet and Rinkeby. Please connect to Mainnet or Rinkeby.");
}
...
function showDocument(id) {
    ...
    docGetEevent = ethblogInstance.DocumentEvent({ docid: id }, { fromBlock: startBlockNo, toBlock: 'latest' });
    ...
    commentSearchEvent = ethblogInstance.DocumentEvent({ refid: id }, { fromBlock: startBlockNo, toBlock: 'latest' });
...
// Sends a document of the logged in user with refid, title, tags and text to the ethblog contract
            ...
            if (!error) {
                ...
                web3.eth.getBlock(blocknr, function (error, block) {
                    ...
                    if (!error) {
                        ...
                        ethblogInstance.documentIt(refid, timestamp, taghasharray, tags, title, text, function (error, txhash) {
